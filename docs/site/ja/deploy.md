...
>
> **自分で名前を付けた環境（テスト用の `dbtest` など）は使えます。**
> **`application.<その名前>.conf` があれば警告は出ません**——ファイルを置いたこと自体が、
> その名前を意図して付けた印だからです。打ち間違いのほうは、その綴りのファイルがありません。
> どちらの場合も、判定はどれにも当たりません。
>
