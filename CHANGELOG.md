...
| | |
|---|---|
| **失敗したら、`rollback()` するまでその先は1文も通りません**（D-156） | 各文の `catch` が**その場で `rollback()` を呼んでいた**のをやめました。**トランザクションの外では1行も仕事をしていませんでした**が、中では別の意味がありました——PostgreSQL は1文でも失敗すると **ROLLBACK するまで以降の文を全部断る**ので、**「エラーが出ても続けて、残りをコミットする」という動きは、枠組みが黙って巻き戻していたから成立していた**のです。それが部分コミットの正体でした。いまは**巻き戻すかどうかを呼んだ側が決めます**——`SQL → commit → SQL（失敗）→ rollback → SQL → commit` と分岐して続けられます（`rollback()` がエラーの持ち越しも畳みます）。書き方は [トランザクション](https://jimble.io/ja/transaction) に |
...
| **トランザクションの中でエラーが出たら、コミットしなくなりました**（D-155 / D-156） | jimble の DB は**エラーを例外ではなく戻り値で返します**。中の `db.update(...)` が `-1` を返しても処理は正常に終わったように見えるので、**そのままコミットしていました**。しかも失敗した文の中で枠組みが `rollback()` を呼ぶので、**そこまでの文は巻き戻り、そこから先の文だけがコミットされる**——例外もログも出ないまま、**データが半分だけ入る**壊れ方でした。いまは **`DB.commit()`** がロールバックして `CodeException`（`DB_004`）を投げます（`DBTransaction` を通さず `db.beginTransaction()` から直に書いた道も同じです）。**判定はトランザクション全体を見ています**（`db.isError()` は直前の1文しか見ないので、失敗のあとに成功する文が1つあるだけでは隠れません）。エラーを承知のうえで通したいときは **`transaction.acceptError()`** を呼んでください。書き方は [トランザクション](https://jimble.io/ja/transaction) に |
