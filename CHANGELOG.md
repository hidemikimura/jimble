...
| | |
|---|---|
| **二要素認証（TOTP）が入りました**（F-W-32 / D-153） | パスワードが合ったあと `Mfa.isActive(利用者 ID)` なら `Mfa.pending(context, principal)` でコードの画面へ送り、`Mfa.complete(context, code)` が通ったときだけログインします。**コードを入れるまでは「ログインしていない」扱いです**——`Auth.principal` は `Principal.ANONYMOUS` を返すので、**ルートを足した人が何もしなければ入れません**。登録は `Mfa.enroll(利用者 ID, 表示名)` が `otpauth://` の URI と回復コードを返し、**`Mfa.activate` でコードが合って初めて有効になります**（ここを1段にすると、**QR の読み取りに失敗した人が二度と入れなくなります**）。**秘密鍵は `cipher.key` で暗号化して持ちます。鍵が設定されていなければ `enroll` は断ります**——TOTP の秘密鍵はパスワードのハッシュと違い、**漏れたらその場でコードが作れます**（破る手間がゼロです）。**回復コードを 10 個出します**（DB には SHA-256 だけ。1つは1回だけ。残りは `Mfa.remainingRecoveryCodes`）。**一度通ったコードは、その窓が終わるまで通りません**（覗き見たコードの入れ直しを止めます）。総当たりは既存の `Lockout` で抑えます——**6桁は 100 万通りしかないので、抑えないと1日で当たります**。`Mfa.disable` は **`attribute(Auth.FULL_AUTH, true)` を付けたルートから**呼んでください（緩いと、**Cookie を盗んだ側が2要素を外せます**）。QR 画像は作りません（URI を渡すので画面側で描いてください。依存を増やさないためです）。SMS とメールはありません（SMS は **SIM の乗っ取りで抜かれます**）。設定は `auth.mfa.*`、書き方は [ログインと認可](https://jimble.io/ja/auth) に |
